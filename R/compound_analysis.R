...
plot_ly(layout, x = ~X1, y = ~X2, z = ~X3, type="scatter3d", mode='markers',
...
layout$cluster_name <- as.factor(layout$cluster)
head(layout)
fig3 <- plot_ly(layout, x = ~X1, y = ~X2, z = ~X3, type="scatter3d", mode='markers',
...
head(merged)
plot_ly(merged, x = ~X1, y = ~X2, z = ~X3, type = "scatter3d", colors = bigpal,
        mode='markers', text = ~molecule, color = ~hbscan_cluster)

new_sub <- merged[merged$molecule %in% c("Scriptaid", "Vorinostat"),]
new_sub
plot_ly(new_sub, x = ~X1, y = ~X2, z = ~X3, type="scatter3d", colors = bigpal,
